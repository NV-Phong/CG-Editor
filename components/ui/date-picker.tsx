...
export function DatePicker({ className }: React.HTMLAttributes<HTMLDivElement>) {
  const { setTotalDays } = useContributionGraphContext();
  ...
  const totalDays = React.useMemo(() => calculateTotalDays(dateSelections), [dateSelections, calculateTotalDays]);
  ...
  return (
    <div className={cn("grid gap-2 w-fit", className)}>
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-auto p-3 z-[51] relative" align="center">
          {calendarComponent}
          <div className="flex gap-4 pt-2">
            <div className="flex-1 min-w-[260px]">
              <DateSelectionList dateSelections={dateSelections} onRemove={handleRemoveSelection} />
            </div>
            <div className="flex flex-col gap-2 min-w-[200px]">
              ...
              <div className="flex items-center justify-end gap-3">
                <Button variant="outline" onClick={handleClear} className="w-20">
                  Clear
                </Button>
